...
# Gespreksnotities per e-mail

Na ieder afgerond ElevenLabs-gesprek accepteert de service een beveiligde post-call webhook op:

```text
https://my-call-agent-lgm3.onrender.com/elevenlabs/post-call
```

De webhook controleert de ElevenLabs HMAC-handtekening, negeert gesprekken van andere agents en stuurt de samenvatting, verzamelde gegevens en het transcript via Resend naar `EMAIL_TO`. Een idempotency-key op basis van het conversation ID voorkomt dubbele e-mails bij een retry.

Voeg in Render onder **Environment** exact deze variabelen toe:

```text
ELEVENLABS_WEBHOOK_SECRET=het HMAC-secret van de actieve ElevenLabs-workspace-webhook
RESEND_API_KEY=re_...
EMAIL_FROM=Fleur <onboarding@resend.dev>
EMAIL_TO=jouw-eigen-emailadres@example.com
```

Let op: variabelen zijn hoofdlettergevoelig. Gebruik dus `RESEND_API_KEY`, niet `Resend_API_Key`. Met `onboarding@resend.dev` kan Resend tijdens de testfase alleen verzenden naar het e-mailadres van je eigen Resend-account. Voor andere ontvangers verifieer je eerst een eigen domein in Resend en wijzig je `EMAIL_FROM`.

In ElevenLabs laat je precies één workspace-webhook actief met bovenstaande URL en HMAC-authenticatie. Kopieer het secret van diezelfde actieve webhook naar Render. Verwijder of deactiveer dubbele webhooks, anders kan ieder gesprek tweemaal worden aangeboden.

Na wijzigingen in Render kies je **Manual Deploy → Deploy latest commit**. De webhookroute is pas beschikbaar nadat de nieuwe GitHub-bestanden succesvol zijn gedeployed.
